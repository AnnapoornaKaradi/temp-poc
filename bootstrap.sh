...
sudo az storage blob download --container-name "cortex" --file "/tmp/EXOS_DEB_Non-Persistent_8-9.tar.gz" --name "EXOS_DEB_Non-Persistent_8-9.tar.gz" --account-name ${storage_account_name} --sas-token "${sas_token}" || error "Azure blob storage donload command failed"
sudo mkdir -p /tmp/cortex
sudo tar -zvxf /tmp/EXOS_DEB_Non-Persistent_8-9.tar.gz -C /tmp/cortex || error "Extractin Cortext file failed"
sudo mkdir -p /etc/panw 
sudo mv /tmp/cortex/cortex.conf /etc/panw/ || error "Moving cortex.conf to /etc/panw failed"
sudo chmod -x /tmp/cortex/cortex-8.9.0.131661.sh  || error "Add execute perms to cortext script"
sudo /usr/bin/bash /tmp/cortex/cortexInstaller.sh -- --vm-template || error "Error installing Cortex"
